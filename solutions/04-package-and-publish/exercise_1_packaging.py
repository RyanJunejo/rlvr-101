"""
ANSWER KEY -- Unit 04, Lab 1 — packaging a task so someone else can use it.
...
def load_environment(num_problems: int = 100, seed: int = 0, **kwargs) -> vf.Environment:
    ...
    return vf.SingleTurnEnv(
        dataset=build_dataset(num_problems, seed),
        eval_dataset=build_dataset(num_problems, seed),
        system_prompt=SYSTEM_PROMPT,
        rubric=vf.Rubric(funcs=[correct_answer], weights=[1.0]),
        **kwargs,
    )
...
name = "{name}"
version = "0.1.0"
description = "Two-digit multiplication, scored by exact match."

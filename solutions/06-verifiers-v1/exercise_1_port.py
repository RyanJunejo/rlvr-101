"""
ANSWER KEY -- Unit 06, Lab 1 — port the Unit 02 math task to v1.
...
class MathTask(vf.Task[MathData, vf.State, vf.TaskConfig]):
    ...
    @vf.reward
    async def correct_answer(self, task: MathData, trace: vf.Trace) -> float:
        """Exact match on the final Answer: line.

        `async` is not optional. A sync method here raises "An asyncio.Future,
        a coroutine or an awaitable is required" from inside Task.score, which
        does not read like "you forgot async".
        """
    ...
    @vf.reward(weight=0.2)
    async def has_answer_line(self, trace: vf.Trace) -> float:
        """Format only. Asks for `trace` and not `task`, because it doesn't need
        the answer -- argument injection means you request exactly what you use."""

"""
ANSWER KEY -- Unit 02, Lab 3 — the reward hacking lab.
...
class AttackTask(vf.Task[AttackData, vf.State, vf.TaskConfig]):
    ...
    async def robust_correctness(self, task: AttackData, trace: vf.Trace) -> float:
        ...
        if not matches:                                      # rule 4: no slot, no credit
            return 0.0
        claim = matches[-1].strip().rstrip(".").strip()      # rules 1 + 2
        return 1.0 if claim == task.answer.strip() else 0.0  # rule 3: ==, never `in`

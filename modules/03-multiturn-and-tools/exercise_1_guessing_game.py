...
def parse_guess(text: str) -> int | None:
    ...
    """
    # TODO: last GUESS_RE match as int, else None.
    return todo("parse_guess: last 'Guess: <n>' match, or None")
...
def respond(guess: int, secret: int) -> str:
    ...
    return todo("respond: 'Too low.' / 'Too high.' / 'Correct!'")
...
class GuessTask(vf.Task[GuessData, vf.State, vf.TaskConfig]):
    ...
    # TODO: write `solved` (@vf.reward) and `num_guesses` (@vf.metric).

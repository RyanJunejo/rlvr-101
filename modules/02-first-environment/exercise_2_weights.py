"""
Lab 2 — weights, and where the judgment lives.
...
def describe_rewards(task: vf.Task) -> list[tuple[str, float]]:
    ...
    attribute `_vf_weight` -- found by reading the decorator's source, and the
    leading underscore is a warning that a later verifiers may rename it.

        return [(fn.__name__, fn._vf_weight) for fn in ...]
    ...
    return todo("describe_rewards: [(name, weight)] from discover_decorated")

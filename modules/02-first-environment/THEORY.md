...
```
U(x)   the true objective — how good output x really is
```
...
```
R(x)   the proxy — what your scoring function returns
```

Training maximizes `R`. It has no access to `U` at all. So the entire question
of reward design is: **over the region where training will actually push, does
`R` still track `U`?**

Note what that question is *not*. It isn't "is `R` a good measurement of `U`
on typical inputs" — your grader was fine on typical inputs; it scored 5/12 only
...
|---|---|
| **noise** — `R = U + ε`, symmetric error | averages out; more samples help |
...
**Regressional.** `R` = `U` + noise. Selecting the highest `R` selects partly for
high `U` and partly for lucky noise, so the winner's true quality is
...
without any intent — gradient ascent on `R` is a search for `R`'s maximum, and
that search is functionally adversarial toward `R`'s defects.
...
That's a genuinely stronger position, and it's why RLVR became the dominant
recipe for math and code.
...
- **Answer leakage.** If the prompt contains the answer anywhere, copying it is
  a legitimate maximum of `R` and a total failure of `U`.
...
a strictly larger set. Every extra accepted string is an extra place the proxy
can exceed the truth.
